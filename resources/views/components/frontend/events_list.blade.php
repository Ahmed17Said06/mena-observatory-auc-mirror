@foreach($events as $event)
        ...
        <div class="col-lg-8 d-flex flex-column flex-lg-row">
            ...
            <div class='col-lg-8'>
                ...
                <div class="d-none d-lg-flex">
                    <a class="notify_me btn" target="_blank"   href="http://www.google.com/calendar/event?action=TEMPLATE&text={{urlencode($event['title'])}}&dates={{str_replace(' ', '', date('Y m d \T h m s Z', strtotime($event['start_date'])))}}/{{str_replace(' ', '', date('Y m d \T h m s Z', strtotime($event['end_date'])))}}&details={{urlencode($event['description'])}}&location={{urlencode($event['gmaps_url'])}}"><i class="fa fa-bell"></i> Notify Me</a>
                    <livewire:save-event-with-text :eventId="$event['id']" :wire:key="'event-1-'.$event['id']"/>
    ...
    <div class="d-block d-lg-none">
        ...
        <livewire:save-event-with-text :eventId="$event['id']"  :wire:key="'event-2-'.$event['id']"/>
